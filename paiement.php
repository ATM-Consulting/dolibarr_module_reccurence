...
$id_charge = GETPOST("id",'int');
$action = GETPOST('action','alpha');
$amounts = array();

$TRecurrences = GETPOST('recurrences','array'); // Tableau des récurrences à payer
...
if ($action == 'add_payment')
{
	...
	if (! $error)
	{
        ...
        if (! $error)
        {
			...
			$TSelected_charges = GETPOST('selected_charges','array');
...
if (!empty($TRecurrences)) {
	...
	print '</td>';
	print '<td colspan="3"><input name="num_paiement" type="text" value="'.GETPOST('num_paiement','alpha').'"></td></tr>'."\n";
